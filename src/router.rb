...
module Router
  def self.route(req, env = {})
    ...
    when 'user'
      return query[1].nil? ? not_found(req) : Users.get_user(req, query[1], user)
    when 'submit'
      if user[:id].nil?
        return index(req, 302, {'Location' => '/'}, user)
      else
    ...
    when 'thread'
      return query[1].nil? ? not_found(req) : Threads.get_thread(req, query[1], user)
    else
  ...
  def self.index(req, status = 200, headers = {}, user = {})
    threads = Threads.get_threads() # retrieve threads
